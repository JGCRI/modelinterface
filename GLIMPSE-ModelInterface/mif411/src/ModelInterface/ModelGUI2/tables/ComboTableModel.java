...
public class ComboTableModel extends BaseTableModel {
	...
	/**
	 * Constructor initializes data members, and calls buildTable to create the
	 * arrays of data maps, and the path vectors for each data map. Creates the
	 * headers for the table axis and path, , also creates the filterMaps based on
	 * the path information.
	 * 
	 * @param tp the Tree Path which was selected from the tree, needed to build
	 *           table doc needed to run the XPath query against tableTypeString to
	 *           be able to display the type of table this is
	 */
	...
	/**
	 * Switches the row and column headers, and names. Also sets a boolean so we
	 * know it has been flipped, since it makes a difference how we reference into
	 * the data maps
	 * 
	 * @param row not used here col not used here
	 */
	@Override
	...
	protected void buildTable(XPathExpression xpe) {
		...
		for (int i = 0; i < res.getLength(); ++i) {
			...
			addToDataTree(tempNode, dataTree).put((String) regionAndYear[0] + ";" + (String) regionAndYear[1],
					tempNode);
			if (units == null) {
	...
	/**
	 * Gets path information from a node in a similar fashions as the
	 * MultiTableModel
	 * 
	...
	private Object[] getRegionAndYearFromNode(Node n, Map filterMaps) {
		...
		do {
			if (n.getNodeName().equals(wild.get(0)) || n.getNodeName().equals(wild.get(1))) {
				...
				/*
				 * } else if(!getOneAttrVal(n).equals("fillout=1")) { ret.add(getOneAttrVal(n));
				 * } else { ret.add(getOneAttrVal(n, 1)); }
				 */
	...
	/**
	 * Sorts nodes in a map of maps creating a tree of data, same as in
	 * MulitTableModel.
	 * 
	...
	private Map addToDataTree(Node currNode, Map dataTree) {
		...
		if (((((String) wild.get(0)).matches(".*[Ss]ector") || ((String) wild.get(1)).matches(".*[Ss]ector")))
				&& currNode.getNodeName().matches(".*[Ss]ector")) {
			return tempMap;
		}
		if (currNode.hasAttributes() && !currNode.getNodeName().equals(wild.get(0))
				&& !currNode.getNodeName().equals(wild.get(1))) {
			String attr = getOneAttrVal(currNode);
			/*
			 * if(attr.equals("fillout=1")) { attr = getOneAttrVal(currNode, 1); }
			 */
	...
	/**
	 * Similar to MultiTable model, except instead of creating a table adds the data
	 * to a vector of data maps, and splits the path string up into a vector for
	 * path info.
	 * 
	 * @param dataTree the mappings of attrubutes which will get us to the data
	 * @param parent   so that we can get the data map which is a level up once we
	 *                 hit the bottom
	 * @param regions  column axis attrubutes
	 * @param years    row axis attributes
	 * @param title    a string describing the path in which the data in the table
	 *                 is coming from
	...
	private void recAddTables(Map dataTree, Map.Entry parent, Set regions, Set years, String titleStr) {
		...
		while (it.hasNext()) {
			...
			if (me.getValue() instanceof Node || me.getValue() instanceof Double || me.getValue() instanceof String) {
				TreeMapVector.add(parent.getValue());
	...
	/**
	 * Gets the Key needed to reference into the data map, given a row and col
	...
	private String getKey(int row, int col) {
		...
		// need to mod by the number of data blocks we have to get the correct key for
		// the row
		// and have to take into account the additional column headers for path
		if (flipped) {
			return (String) indRow.get(row % (indRow.size())) + ";"
					+ (String) indCol.get(col - leftHeaderVector.size());
		}
		String s = (String) indCol.get(col - leftHeaderVector.size()) + ";"
				+ (String) indRow.get(row % (indRow.size()));
		return s;
	}
	 ...
	 * 
	 * @param row the row position in the table col the col position in the table
	...
	public Object getValueAt(int row, int col) {
		try {
			...
			if (col < leftHeaderVector.size()) {
				return ((Vector) leftSideVector.get(((Integer) activeRows.get(row)).intValue() / (indRow.size())))
						.get(col);
	...
	/**
	 * Determine if there is a real data value pointed to at at the passed in row
	 * and col. This equates to checking in the correct Map in TreeMapVector and see
	 * if the key at this pos exists.
	 * 
	...
	protected boolean hasValueAt(int row, int col) {
		return col > leftHeaderVector.size()
				&& ((Map) TreeMapVector.get(((Integer) activeRows.get(row)).intValue() / (indRow.size())))
						.containsKey(getKey((Integer) activeRows.get(row), col));
	}

	/**
	 * returns the actual Node that is contained at the position row, col in the
	 * table
	 * 
	 * @param row position in table col position in table
	 ...
	 * @return the header value in the column index, or path index at the position
	 *         passed in
	 */
	@Override
	...
	/**
	 * Get the class for the data that will be in the specified class. This will
	 * return String.class for the left lable columnd and the last column which is
	 * the units column elseDouble.class for the rest.
	 * 
	 * @param columnIndex The column being queried.
	 * @return Double.class for leftHeaderVector.size() &lt; columnIndex &lt;
	 *         getColumnCount()-1 else String.class
	 */
	@Override
	public Class getColumnClass(int columnIndex) {
		return (columnIndex > leftHeaderVector.size()) && (columnIndex < getColumnCount() - 1) ? Double.class
				: String.class;
	}
	 ...
	 * 
	 * @param row the row position being queryed col the column position being
	 *            queryed
	...
	/**
	 * Update the activeRows vector by going through all of the path information,
	 * and the filterMaps and determining if any of the rows correspond the the
	 * qualifying path info.
	 * 
	 * @param possibleFilters the list of nodeNames in the filterMap that wil be
	 *                        fillered
	 */
	@Override
	protected void doFilter(Vector possibleFilters) {
		...
		for (int i = 0; i < possibleFilters.size(); i++) {
			...
			currKeys = (String[]) ((Map) tableFilterMaps.get(possibleFilters.get(i))).keySet().toArray(new String[0]);
			// for (Iterator it = activeRows.iterator(); it.hasNext(); rowPos =
			// (Integer)it.next()) {
			Iterator it = activeRows.iterator();
			while (it.hasNext()) {
				rowPos = (Integer) it.next();
				for (int j = 0; j < currKeys.length; j++) {
					if (!((Boolean) ((Map) tableFilterMaps.get(possibleFilters.get(i))).get(currKeys[j]))
							.booleanValue()) {
						if (((String) ((Vector) leftSideVector.get(rowPos.intValue() / (indRow.size())))
								.get(possibleFilters.size() - i - 1)).equals(currKeys[j])) {
							it.remove();
							break;
	...
	public void setValueAt(Object val, int row, String colname) {
	...
	public void setValueAt(Object val, int row, int col) {
		...
		if (n != null) {
			...
			// Dan: added the try catch to address an issue where set value not working
			// correctly
			try {
			...
			/*
			 * for(int i = 0; i < getRowCount() && ( updown =
			 * ((Node)((TreeMap)TreeMapVector.get( ((Integer)activeRows.get( i )).intValue()
			 * / (indRow.size()))).get( getKey( i, col ) ))) /*(Node)data.get(getKey(i,
			 * col))/ == null; ++i) { }
			 */
			...
			Node parent = (side.getParentNode());
			...
			if (flipped) {
				...
				/*
				 * attributesLineArray[0] = attributesLineArray[1]; attributesLineArray[1] =
				 * temp;
				 */
				temp = headerone;
				headerone = headertwo;
			...
			for (int i = nodePath.size() - 1; i >= 0; --i) {
				...
				if (temp.getNodeName().equals(headertwo)) {
					...
					if (attrTemp == null) {
						...
						parent = checkPath(parent, temp.getNodeName(), attrTemp.getNodeName(), attrTemp.getNodeValue(),
								setEdit);
					}
				}
			}
	...
	/**
	 * Used to follow the path down a tree where parent is the current parent and
	 * want to go under the node with the passed in node name and attributes will
	 * create the node if it does not exsit
	 * 
	...
	public ComboTableModel(QueryGenerator qgIn, Object[] scenarios, Object[] regions, QueryBinding singleBinding,
			DbProcInterrupt interrupt) throws Exception {
		qg = qgIn;
		...
		if (singleBinding == null) {
			buildTable(XMLDB.getInstance().createQuery(qgIn, scenarios, regions, interrupt), qgIn.isSumAll(), isTotal,
					isGlobal);
		} else {
			...
			buildTable(XMLDB.getInstance().createQuery(singleBinding, scenarios, regions, interrupt), qgIn.isSumAll(),
					isTotal, isGlobal);
		}
		...
		Collection<String> rewriteValues = singleBinding == null && qg.shouldAppendRewriteValues()
				? qg.getNodeLevelRewriteMap().values()
				: null;
		for (int i = 0; i < (leftSideVector.size() * indRow.size()); i++) {
			boolean allNulls = true;
			for (int col = leftHeaderVector.size() + 1; col < getColumnCount() && allNulls; ++col) {
				if (((Map) TreeMapVector.get(i / (indRow.size()))).get(getKey(i, col)) != null) {
					allNulls = false;
				}
			}
	...
	private void buildTable(QueryProcessor queryProc, boolean sumAll, boolean isTotal, boolean isGlobal)
			throws Exception {
		// System.out.println("In Function: "+System.currentTimeMillis());

		// sumAll=true; //Dan: Test
		// isTotal=true; //Dan: Test
		...
		try {
			while ((tempNode = (ANode) res.next()) != null) {
				...
				if (axisValues.getKey() == null || axisValues.getValue() == null) {
					...
					throw new Exception(
							"<html><body>Could not determine how to categorize the results.<br> Please check your axis node values.</body></html>");
				}
		...
		if (qg.shouldAppendRewriteValues()) {
			for (Iterator<String> rewriteValueIt = rewriteMap.values().iterator(); rewriteValueIt.hasNext();) {
				...
				// the empty string is a special case which meant that we wanted to delete that
				// row
	...
	public void exportToExcel(HSSFSheet sheet, HSSFWorkbook wb, HSSFPatriarch dp) {
		...
		try {
			// for a good chart, the number of rows labeling the cart is added the the
			// 350*350 square
			...
			short colSpan = (short) (imgWidth
					/ (pixelWidthPerChar * (sheet.getColumnWidth((short) (getColumnCount() + 1)))) + firstCol);
			short rowSpan = (short) ((imgHeight / (sheet.getDefaultRowHeightInPoints() * 5 / 3)) + firstRow);

			// WARNING: This is a hack because of java some how looking to load some class
			// that did
			// not exist. Instead of using the utilities which uses the Factory which uses
			// the
